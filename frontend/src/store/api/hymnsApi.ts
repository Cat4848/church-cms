...
export const hymnsApi = createApi({
  ...
  endpoints: (builder) => ({
    getAllHymns: builder.query<Hymn[], string | undefined>({
      query: (searchLyrics: string | undefined) => {
        if (searchLyrics) {
          const searchParams = new URLSearchParams();
          searchParams.append("searchLyrics", searchLyrics);
          return `/hymns?${searchParams.toString()}`;
        } else {
          return "/hymns";
        }
      },
