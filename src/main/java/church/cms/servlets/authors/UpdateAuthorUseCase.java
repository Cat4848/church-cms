...
public class UpdateAuthorUseCase {
  ...
  public UpdateAuthorUseCase(ObjectMapper objectMapper, AuthorRepository authorRepository, Logger logger) {
  ...
  public void execute(HttpServletRequest req, HttpServletResponse res) throws IOException, SQLException {
    ...
    try (ValidatorFactory vf = Validation.buildDefaultValidatorFactory()) {
      ...
      Set<ConstraintViolation<UpdateAuthorPayload>> violations = validator.validate(payload);

      if (!violations.isEmpty()) {
        int size = violations.size();
        int i = 0;
        StringBuilder sb = new StringBuilder();
        for (ConstraintViolation<UpdateAuthorPayload> violation : violations) {
      ...
      objectMapper.writeValue(res.getWriter(), updatedAuthor);
